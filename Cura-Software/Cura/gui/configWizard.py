...
class FirstInfoPage(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(language.getText('Welcome, and thanks for trying Cura!'))
		self.AddSeperator()
		self.AddText(language.getText('This wizard will help you with the following steps:'))
		self.AddText(language.getText('* Configure Cura for your machine'))
		self.AddText(language.getText('* Upgrade your firmware'))
		self.AddText(language.getText('* Check if your machine is working safely'))
		self.AddText(language.getText('* Level your printer bed'))
...
class RepRapInfoPage(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(language.getText(
			'RepRap machines are vastly different, and there is no\ndefault configuration in Cura for any of them.'))
		self.AddText(language.getText('If you like a default profile for your machine added,\nthen make an issue on github.'))
		self.AddSeperator()
		self.AddText(language.getText('You will have to manually install Marlin or Sprinter firmware.'))
		self.AddSeperator()
		self.machineWidth = self.AddLabelTextCtrl(language.getText('Machine width (mm)'), '80')
		self.machineDepth = self.AddLabelTextCtrl(language.getText('Machine depth (mm)'), '80')
		self.machineHeight = self.AddLabelTextCtrl(language.getText('Machine height (mm)'), '60')
		self.nozzleSize = self.AddLabelTextCtrl(language.getText('Nozzle size (mm)'), '0.5')
		self.heatedBed = self.AddCheckbox(language.getText('Heated bed'))
		self.HomeAtCenter = self.AddCheckbox(language.getText('Bed center is 0,0,0 (RoStock)'))
...
class MachineSelectPage(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(language.getText('What kind of machine do you have:'))
		...
		self.OtherRadio = self.AddRadioButton(language.getText("Other (Ex: RepRap)"))
		self.OtherRadio.Bind(wx.EVT_RADIOBUTTON, self.OnOtherSelect)
		self.AddSeperator()
		self.AddText(language.getText('The collection of anonymous usage information helps with the continued improvement of Cura.'))
		self.AddText(language.getText('This does NOT submit your models online nor gathers any privacy related information.'))
		self.SubmitUserStats = self.AddCheckbox(language.getText('Submit anonymous usage information:'))
		self.AddText(language.getText('For full details see: http://wiki.ultimaker.com/Cura:stats'))
...
class SelectParts(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(language.getText('To assist you in having better default settings for your Ultimaker\nCura would like to know which upgrades you have in your machine.'))
		self.AddSeperator()
		self.springExtruder = self.AddCheckbox(language.getText('Extruder drive upgrade'))
		self.heatedBed = self.AddCheckbox(language.getText('Heated printer bed (self built)'))
		self.dualExtrusion = self.AddCheckbox(language.getText('Dual extrusion (experimental)'))
		self.AddSeperator()
		self.AddText(language.getText('If you have an Ultimaker bought after october 2012 you will have the\nExtruder drive upgrade. If you do not have this upgrade,\nit is highly recommended to improve reliability.'))
		self.AddText(language.getText('This upgrade can be bought from the Ultimaker webshop\nor found on thingiverse as thing:26094'))
...
class FirmwareUpgradePage(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(
			language.getText('Firmware is the piece of software running directly on your 3D printer.\nThis firmware controls the step motors, regulates the temperature\nand ultimately makes your printer work.'))
		self.AddHiddenSeperator()
		self.AddText(
			language.getText('The firmware shipping with new Ultimakers works, but upgrades\nhave been made to make better prints, and make calibration easier.'))
		self.AddHiddenSeperator()
		self.AddText(
			language.getText('Cura requires these new features and thus\nyour firmware will most likely need to be upgraded.\nYou will get the chance to do so now.'))
		upgradeButton, skipUpgradeButton = self.AddDualButton(language.getText('Upgrade to Marlin firmware'), language.getText('Skip upgrade'))
...
class UltimakerCheckupPage(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(
			language.getText('It is a good idea to do a few sanity checks now on your Ultimaker.\nYou can skip these if you know your machine is functional.'))
		b1, b2 = self.AddDualButton(language.getText('Run checks'), language.getText('Skip checks'))
		b1.Bind(wx.EVT_BUTTON, self.OnCheckClick)
		b2.Bind(wx.EVT_BUTTON, self.OnSkipClick)
		self.AddSeperator()
		self.commState = self.AddCheckmark(language.getText('Communication:'), self.unknownBitmap)
		self.tempState = self.AddCheckmark(language.getText('Temperature:'), self.unknownBitmap)
		self.stopState = self.AddCheckmark(language.getText('Endstops:'), self.unknownBitmap)
		self.AddSeperator()
		self.infoBox = self.AddInfoBox()
		self.machineState = self.AddText('')
		self.temperatureLabel = self.AddText('')
		self.errorLogButton = self.AddButton(language.getText('Show error log'))
	...
	def OnCheckClick(self, e=None):
		...
		if self.comm is not None:
			...
			return
		self.infoBox.SetBusy(language.getText('Connecting to machine.'))
	...
	def mcTempUpdate(self, temp, bedTemp, targetTemp, bedTargetTemp):
		...
		if self.checkupState == 0:
			...
			if temp[self.checkExtruderNr] > 70:
				self.checkupState = 1
				wx.CallAfter(self.infoBox.SetInfo, language.getText('Cooldown before temperature check.'))
				self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
				self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
			else:
				self.startTemp = temp[self.checkExtruderNr]
				self.checkupState = 2
				wx.CallAfter(self.infoBox.SetInfo, language.getText('Checking the heater and temperature sensor.'))
			...
			if temp[self.checkExtruderNr] > self.startTemp + 40:
				...
				if self.tempCheckTimeout < 1:
					...
					wx.CallAfter(self.infoBox.SetError, language.getText('Temperature measurement FAILED!'), 'http://wiki.ultimaker.com/Cura:_Temperature_measurement_problems')
					self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
					self.comm.sendCommand('M104 S0 T%d' % (self.checkExtruderNr))
	...
	def mcStateChange(self, state):
		...
		if self.comm.isOperational():
			...
			wx.CallAfter(self.machineState.SetLabel, language.getText('Communication State: %s') % (self.comm.getStateString()))
		elif self.comm.isError():
			wx.CallAfter(self.commState.SetBitmap, self.crossBitmap)
			wx.CallAfter(self.infoBox.SetError, language.getText('Failed to establish connection with the printer.'), 'http://wiki.ultimaker.com/Cura:_Connection_problems')
		...
		if self.checkupState >= 3 and self.checkupState < 10 and ('_min' in message or '_max' in message):
			...
			if self.checkupState == 3:
				if not self.xMinStop and not self.xMaxStop and not self.yMinStop and not self.yMaxStop and not self.zMinStop and not self.zMaxStop:
					self.checkupState = 4
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the right X endstop.'))
				...
				if not self.xMinStop and self.xMaxStop and not self.yMinStop and not self.yMaxStop and not self.zMinStop and not self.zMaxStop:
					self.checkupState = 5
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the left X endstop.'))
				...
				if self.xMinStop and not self.xMaxStop and not self.yMinStop and not self.yMaxStop and not self.zMinStop and not self.zMaxStop:
					self.checkupState = 6
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the front Y endstop.'))
				...
				if not self.xMinStop and not self.xMaxStop and self.yMinStop and not self.yMaxStop and not self.zMinStop and not self.zMaxStop:
					self.checkupState = 7
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the back Y endstop.'))
				...
				if not self.xMinStop and not self.xMaxStop and not self.yMinStop and self.yMaxStop and not self.zMinStop and not self.zMaxStop:
					self.checkupState = 8
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the top Z endstop.'))
				...
				if not self.xMinStop and not self.xMaxStop and not self.yMinStop and not self.yMaxStop and self.zMinStop and not self.zMaxStop:
					self.checkupState = 9
					wx.CallAfter(self.infoBox.SetAttention, language.getText('Please press the bottom Z endstop.'))
				...
				if not self.xMinStop and not self.xMaxStop and not self.yMinStop and not self.yMaxStop and not self.zMinStop and self.zMaxStop:
					...
					self.comm.close()
					wx.CallAfter(self.infoBox.SetInfo, language.getText('Checkup finished'))
...
class LanguageWizard(wx.wizard.Wizard):
	def __init__(self):
		super(LanguageWizard, self).__init__(None, -1, "Language Wizard")

		self.Bind(wx.wizard.EVT_WIZARD_PAGE_CHANGED, self.OnPageChanged)
		self.Bind(wx.wizard.EVT_WIZARD_PAGE_CHANGING, self.OnPageChanging)

		self.language = LanguageSelectionPage(self)

		self.FitToPage(self.language)
		self.GetPageAreaSizer().Add(self.language)

		self.RunWizard(self.language)
		self.Destroy()
...
class LanguageSelectionPage(InfoPage):
	def __init__(self, parent):
		super(LanguageSelectionPage, self).__init__(parent, "Select your Language")
		self.AddText(language.getText('What kind of language do you want to use:'))
		self.AddText(language.getText('You need to restart Cura the apply the configuration'))

		self.EnglishRadio = self.AddRadioButton("English", style=wx.RB_GROUP)
		self.EnglishRadio.SetValue(True)
		self.EnglishRadio.Bind(wx.EVT_RADIOBUTTON, self.OnEnglishSelect)
		self.ChineseRadio = self.AddRadioButton(u"\u4E2D\u6587")
		self.ChineseRadio.Bind(wx.EVT_RADIOBUTTON, self.OnChineseSelect)
	
	def OnEnglishSelect(self, e):
		language.switchTo("english")

	def OnChineseSelect(self, e):
		language.switchTo("chinese")
		
	def StoreData(self):
		if self.EnglishRadio.GetValue():
			profile.putPreference('language', 'english')
			language.switchTo("english")
		elif self.ChineseRadio.GetValue():
			profile.putPreference('language', 'chinese')
			language.switchTo("chinese")
...
class bedLevelWizardMain(InfoPage):
	def __init__(self, parent):
		...
		self.AddText(language.getText('This wizard will help you in leveling your printer bed'))
		self.AddSeperator()
		self.AddText(language.getText('It will do the following steps'))
		self.AddText(language.getText('* Move the printer head to each corner'))
		self.AddText(language.getText('  and let you adjust the height of the bed to the nozzle'))
		self.AddText(language.getText('* Print a line around the bed to check if it is level'))
		self.AddSeperator()

		self.connectButton = self.AddButton(language.getText('Connect to printer'))
		self.comm = None

		self.infoBox = self.AddInfoBox()
		self.resumeButton = self.AddButton(language.getText('Resume'))
	...
	def OnResume(self, e):
		...
		if self._wizardState == 2:
			wx.CallAfter(self.infoBox.SetBusy, language.getText('Moving head to back left corner...'))
			...
			wx.CallAfter(self.infoBox.SetBusy, language.getText('Moving head to back right corner...'))
			self.comm.sendCommand('G1 Z3 F%d' % (feedZ))
			...
			wx.CallAfter(self.infoBox.SetBusy, language.getText('Moving head to front right corner...'))
			self.comm.sendCommand('G1 Z3 F%d' % (feedZ))
			...
			wx.CallAfter(self.infoBox.SetBusy, language.getText('Heating up printer...'))
	...
	def mcTempUpdate(self, temp, bedTemp, targetTemp, bedTargetTemp):
		if self._wizardState == 1:
			self._wizardState = 2
			wx.CallAfter(self.infoBox.SetAttention, language.getText('Adjust the front left screw of your printer bed\nSo the nozzle just hits the bed.'))
			wx.CallAfter(self.resumeButton.Enable, True)
		elif self._wizardState == 3:
			self._wizardState = 4
			wx.CallAfter(self.infoBox.SetAttention, language.getText('Adjust the back left screw of your printer bed\nSo the nozzle just hits the bed.'))
			wx.CallAfter(self.resumeButton.Enable, True)
		elif self._wizardState == 5:
			self._wizardState = 6
			wx.CallAfter(self.infoBox.SetAttention, language.getText('Adjust the back right screw of your printer bed\nSo the nozzle just hits the bed.'))
			wx.CallAfter(self.resumeButton.Enable, True)
		elif self._wizardState == 7:
			self._wizardState = 8
			wx.CallAfter(self.infoBox.SetAttention, language.getText('Adjust the front right screw of your printer bed\nSo the nozzle just hits the bed.'))
			wx.CallAfter(self.resumeButton.Enable, True)
			...
			if temp[0] < profile.getProfileSettingFloat('print_temperature') - 5:
				...
				wx.CallAfter(self.infoBox.SetInfo, language.getText('Printing a square on the printer bed at 0.3mm height.'))
	...
	def mcStateChange(self, state):
		...
		if self.comm.isOperational():
			if self._wizardState == 0:
				wx.CallAfter(self.infoBox.SetInfo, language.getText('Homing printer...'))
				...
				wx.CallAfter(self.infoBox.SetInfo, language.getText('Calibration finished.\nThe squares on the bed should slightly touch each other.'))
				wx.CallAfter(self.infoBox.SetReadyIndicator)
